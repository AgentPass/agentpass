...
data "terraform_remote_state" "backend" {
  ...
  config = {
    bucket = "agentpass-terraform-state"
    key    = "backend/staging/terraform.tfstate"  # Points to staging backend
    region = var.aws_region
  }
}
...
module "beanstalk" {
  ...
  environment_variables = {
    ...
    REACT_APP_DEBUG      = "true"  # Enable debug mode for staging
    REACT_APP_API_URL    = "https://api-staging.yourdomain.com"  # Staging API Gateway URL
  }
}